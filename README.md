...
SysGuard 是一个基于 Go 与 CloudWeGo Eino 的本地 AI 运维 Agent。它把主机巡检、异常检测、SOP/RAG 检索、LLM tool-use、执行安全、结果验证和历史沉淀放进同一张 Eino graph 中，重点服务于一个始终开启的 AI Agent 闭环。

> 当前项目默认进入 AI Agent 路径。修复仍然 dry-run，危险或有副作用操作仍需要审批，适合先在本地或测试机器观察运行，再逐步接入真实服务和执行权限。
...
SysGuard 不是一个会直接“放飞自我”的自动修复脚本。它的设计目标是成为一个清晰可靠的 AI 运维 Agent：
...
发现异常后，SysGuard 始终进入 LLM Agent 路径。是否真实执行修复由 dry-run、审批和命令安全策略控制。
...
打开页面后点击“立即巡检”，你会看到一次完整的异常 graph run：健康检查失败、异常进入 AI Agent 路径、运行记录写入 `data/runs.json`。详细说明见 [docs/demo.md](docs/demo.md)。
...
1. 本地首次运行保持 `dry_run=true`。
2. 确认巡检、trace、dashboard 和 Agent final 正常。
3. 只让模型使用只读工具观察几轮。
4. 为少量低风险服务开启审批修复。
...
| 场景 | 期望能力 |
| --- | --- |
| `service_down_ai_path` | 异常进入 ReAct 路径，检索 SOP/history，健康检查验证，生成含诊断、验证、回滚的结果 |
