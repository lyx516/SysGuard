...
- **本地可跑**：默认不依赖云端控制面，适合先在测试机观察巡检和 graph run。
- **安全优先**：默认 `dry_run=true`，有副作用工具进入审批队列，批准后才允许继续执行。
- **证据驱动**：LLM 处理异常前会检索结构化 runbook、SOP citation 和历史记录。
- **可审计**：每次 graph run、工具调用、审批请求和处理历史都会落盘，Dashboard/API 可查询。
- **Go + Eino**：适合作为 AI SRE Agent、runbook automation、incident response 的工程骨架。
