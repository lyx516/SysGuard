...
### 0. 修复工作流 (remediation_workflow)

基于历史学习的智能修复工作流，实现规范化的三步修复流程。

**工具**:
- `problem_analyzer` - 问题分析工具
- `plan_executor` - 计划执行工具
- `document_generator` - 文档生成工具

**工作流程**:
1. **问题分析**: 调用 `health_check`、`log_analysis`、`metrics` 等 skills 分析当前环境，确定问题类型和根源
2. **计划执行**: 检索历史文档，参考成功案例，调用 `service_management`、`container_management` 等 skills 执行修复
3. **文档生成**: 如果是首次处理或处理成功，生成 Markdown 格式的处理文档并保存

**核心特性**:
- **历史学习**: 从历史处理记录中学习，提高修复效率
- **相似度搜索**: 基于 Jaccard 相似度算法查找相似案例
- **首次检测**: 自动判断是否为首次处理
- **文档自动生成**: 生成结构化的处理文档
...
output, _ := workflowSkill.Execute(ctx, input)
```
